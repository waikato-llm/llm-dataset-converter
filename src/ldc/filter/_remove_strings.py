...
from ldc.text_utils import remove_strings
from ldc.api import Filter


class RemoveStrings(Filter):
    """
    Removes strings.
    """

    def __init__(self, remove: List[str] = None,
                 location: Union[str, List[str]] = LOCATION_ANY, languages: List[str] = None,
                 logger_name: str = None, logging_level: str = LOGGING_WARNING):
        """
        Initializes the filter.

        :param remove: the list of strings to remove from the text
        :type remove: list
    ...
    def description(self) -> str:
        ...
        :rtype: str
        """
        return "Removes strings from text."
    ...
    def _create_argparser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument("-r", "--remove", type=str, default=None, help="Strings to remove from the text (gets applied before skipping empty lines).", nargs="*")
        add_location_argument(parser, "Where to remove the strings")
    ...
    def _remove_strings(self, line: str) -> Tuple[str, int]:
        """
        Removes the strings.
        ...
        :rtype: list
        """
        result, affected = remove_strings(line.split("\n"), self.remove)
    ...
    def _process(self, data) -> int:
        ...
        if isinstance(data, PairData):
            if locations_match(self.location, LOCATION_INSTRUCTION):
                data.instruction, r = self._remove_strings(data.instruction)
                removed += r
            if locations_match(self.location, LOCATION_INPUT):
                data.input, r = self._remove_strings(data.input)
                removed += r
            if locations_match(self.location, LOCATION_OUTPUT):
                data.output, r = self._remove_strings(data.output)
                removed += r
        elif isinstance(data, ClassificationData):
            if locations_match(self.location, LOCATION_TEXT):
                data.text, r = self._remove_strings(data.text)
                removed += r
        elif isinstance(data, PretrainData):
            if locations_match(self.location, LOCATION_CONTENT):
                data.content, r = self._remove_strings(data.content)
                removed += r
        elif isinstance(data, TranslationData):
            if self.languages is None:
                for k in data.translations:
                    data.translations[k], r = self._remove_strings(data.translations[k])
                    removed += r
            else:
                for lang in self.languages:
                    if lang in data.translations:
                        data.translations[lang], r = self._remove_strings(data.translations[lang])
                        removed += r
        else:
